...
static httpd_handle_t start_webserver(const char *base_path)
{
    ...
    ctx_info->basic_auth_info->username = CONFIG_MODEM_WEB_USERNAME;
    ctx_info->basic_auth_info->password = CONFIG_MODEM_WEB_PASSWORD;
...
static esp_err_t init_fs(void)
{
    esp_vfs_spiffs_conf_t conf = {
        .base_path = CONFIG_MODEM_WEB_MOUNT_POINT,
...
esp_err_t modem_http_init(modem_wifi_config_t *wifi_config)
{
    if (s_if_init == false) {
        ...
        s_server = start_webserver(CONFIG_MODEM_WEB_MOUNT_POINT);
